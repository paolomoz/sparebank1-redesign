  ...
  if (!matched && !ucMatched) {
    const hit = await page.evaluate(() => {
      const LABELS = new Set(['accept', 'accept all', 'allow all', 'agree', 'ok', 'decline',
        ...
        // project instrument fix (sparebank1, 2026-09-14): the SB1 consent dialog's accept label is Norwegian
        // ('Godta alle'); without it the dialog baked into 2/100 captures. Flagged for upstreaming.
        'godta alle', 'godta', 'aksepter alle', 'aksepter']);
      const inOverlay = (el) => {
