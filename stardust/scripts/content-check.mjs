...
const me=PM.find(x=>x.slug===slug); const relToSite=h=>{ if(!me||!/^\.\.?\//.test(h)) return h; const abs='/'+path.posix.normalize(path.posix.join(path.posix.dirname(me.outputPath),h.split(/(?=[?#])/)[0])); return abs; }; const proHrefs=new Set([...pro.querySelectorAll('a[href]')].flatMap(a=>{const h=a.getAttribute('href'); const site=relToSite(h); const search=h.replace(/^https:\/\/www\.sparebank1\.no\/nb\/bank\/[a-z-]+\/kundeservice\.html(?=\?search=)/,''); return [rel(h),rel(site),rel(site).replace(/\.html$/,''),rel(site)+'.html',search];})); // the bundle routes ?search= to the live search page (dynamics #20 interim) — same target
