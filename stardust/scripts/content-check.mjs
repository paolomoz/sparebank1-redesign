...
for(const r of regions){ const root=cap.querySelector(sel[r]); if(!root) continue;
  ...
  for(const a of root.querySelectorAll('a[href]')){ const h=a.getAttribute('href'); if(!h||h==='#/'||h==='#'||h.startsWith('javascript')) continue; if(authUi(a)) continue; if(hidden(a)&&r!=='footer'&&r!=='bank-choice') continue; checked.href++; if(!proHrefs.has(rel(h))) miss.href.push({r,h,t:norm(a.textContent).slice(0,40)}); }
