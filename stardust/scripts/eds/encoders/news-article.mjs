...
export default {
  'article-header': (root, ctx) => {
    ...
    return { html: section([block('hero', ['article', media ? null : 'no-media'], [[media, body]])], { style: 'article-head' }), blocks: ['hero'] };
  },
  'article-body': (root, ctx) => {
    ...
    for (const n of body ? body.children : []) {
      if (n.matches('aside.factbox')) { flush(); parts.push(block('callout', ['fact'], [[prose(n, ctx)]])); blocks.add('callout'); continue; }
    ...
    if (rail) {
      ...
      parts.push(block('cards', ['rail'], rows)); blocks.add('cards');
    }
    return { html: section(parts, { style: styleOf('article', rail ? null : 'no-rail') }), blocks: [...blocks] };
  },
};
