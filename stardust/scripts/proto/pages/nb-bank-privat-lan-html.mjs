...
export function rich(el,{demote=0,h1To=0,brbr=false,emptyB='space',aTrim='strip',legacyImg=false,tidy=false,tidyBase=2}={}){
  if(!el) return '';
  ...
  if(tidy){ let prev=tidyBase; for(const h of [...c.querySelectorAll('h1,h2,h3,h4,h5,h6')]){ let lvl=+h.tagName[1]; if(lvl>prev+1){ lvl=prev+1; const n=c.ownerDocument.createElement('h'+lvl); n.innerHTML=h.innerHTML; h.replaceWith(n); } prev=lvl; } }
...
export const R=(el,o={})=>rich(el,{brbr:true,emptyB:'space',h1To:2,aTrim:'move',tidy:true,...o});
...
/** Round 01: the fill belongs to the cards — no paper-* section tints (max defaults to 0; kept for the API). */
export function assignTints(specs,{max=0}={}){ let n=0; let prev=null; for(const s of specs){ if(s.movement===false){ prev=null; continue; } if(s.wantTint&&n<max&&prev!==s.wantTint){ s.tint=s.wantTint; n++; } prev=s.tint||null; } }

/* ───────────────────────── generic handlers (lån-identical markup) ───────────────────────── */
const doorData=n=>{ const type=n.getAttribute('data-type')||'medium'; const a=n.querySelector('a[href]'); const h=n.querySelector('h1,h2,h3,h4'); const p=n.querySelector('p'); if(type==='small'){ const i=n.querySelector('.nav--small__card__content__icon img'); return {type,href:a?.getAttribute('href'),title:norm(h?.textContent),text:norm(p?.textContent),img:imgSrc(i),alt:i?.getAttribute('alt')||''}; } const bg=n.querySelector('.nav--med__background, .nav--big')?.getAttribute('style')||''; const url=(bg.match(/url\((?:"([^"]*)"|'([^']*)'|([^)]+))/)||[]).slice(1).find(Boolean)||n.getAttribute('data-image-src'); return {type,href:a?.getAttribute('href'),title:norm(h?.textContent),text:norm(p?.textContent),img:url,alt:n.getAttribute('data-image-alt')||''}; };
const titleHtml=(it,cls='h2-l section-title')=>it.title?`\n    <h2 class="${cls}" data-slot="heading">${esc(norm(it.title.textContent))}</h2>${(it.lead||[]).filter(hasText).map(p=>`<p class="lead section-lead" data-slot="text">${inline(p)}</p>`).join('')}`:'';
...
/* ── round 01 bento helpers ── */
/** Span class for n repeated cards: 1 → 12, 2 → 6/6, 3·6·9 → 3-up, otherwise 4-up (tablet halves, mobile stacks via cssBase). */
export const gridCls=n=>n<=1?'grid-1':n===2?'grid-2':(n%3===0&&n%4!==0)?'grid-3':'grid-4';
/** The single action of a card becomes the cover link (whole card clickable, hover system on the card). */
export const coverCta=html=>html.replace(/<p>(<a class="btn [^"]*)"/,'<p class="actions">$1 cover-link"');
/** A photo cell of a bento: the image fills the figure card (object-fit: cover); an SVG sits on a tint instead. */
export const mediaCard=(i,{cls='',eager=false,link,extra=''}={})=>{ const svg=isSvg(imgSrc(i)); return `<figure class="card ${svg?'card--tint media-illu':'media-photo'} ${cls}" data-slot="image">${linked(imgTag(i,{cls:svg?'illu':'media-img',eager,w:svg?0:1280,h:853}),link?{link}:null)}${extra}</figure>`; };
/** Promo row: one li.card per promo — the spot illustration sits on the tint beside the teaser (1 promo spans 12, 2 span 6 each). */
export function promoRow(promos){ const n=promos.length;
  const one=(x)=>{ const src=imgSrc(x.img); const as=x.as||[]; return `<li class="card card--tint promo${src?'':' promo--noart'}${as.length===1?' is-link':''}" data-module="promo-band">${src?`<img class="promo-illu" src="${asset(src)}" alt="" aria-hidden="true" width="160" height="120" loading="lazy" decoding="async">`:''}<div class="card-body">${x.h?`<h2 class="h3" data-slot="heading">${esc(norm(x.h.textContent))}</h2>`:''}${x.p?`<p data-slot="text">${inline(x.p)}</p>`:''}${as.length===1?coverCta(`<p>${btn(as[0],' data-slot="cta"')}</p>`):as.length?`<p class="cta-row">${as.map(a=>btn(a)).join('')}</p>`:''}</div></li>`; };
  return `<ul class="bento promo-grid ${gridCls(Math.min(n,2))}" data-slot="cards">${promos.map(one).join('')}</ul>`; }
/** One column of a captured grid as a card: a leading photo bleeds as .card-image, everything else sits in .card-body. */
function colCard(col,c,base=2){ const parts=[...col.parts]; let head='';
  if(parts[0]?.type==='image'&&!isSvg(imgSrc(parts[0].it.img))){ head=linked(imgTag(parts[0].it.img,{cls:'card-image',w:768,h:432}),parts[0].it); parts.shift(); }
  const single=col.ctas.length===1; let body=colParts({...col,parts},c,base); if(single) body=coverCta(body);
  return `<li class="card card--tint help-col${single?' is-link':''}">${head}<div class="card-body">${body}</div></li>`; }
export const H={
  intro(it,c,{fill='card--frost'}={}){ const back=it.back?`<p class="back"><a class="backlink" href="${esc(it.back.a.getAttribute('href'))}">${icons.back}<span>${esc(norm(it.back.a.textContent))}</span></a></p>`:''; const leads=it.leads.filter(hasText).map(p=>`<p class="lead" data-slot="text">${inline(p)}</p>`).join(''); const ctas=it.ctas.length?`<p class="cta-row">${it.ctas.map((a,i)=>btn(a,i===0?' data-slot="cta"':'')).join('')}</p>`:'';
    const text=`${back}<h1 data-slot="heading">${esc(norm(it.h1.textContent))}</h1>${leads}${ctas}`;
    if(it.img){ const i=it.img.img; const svg=isSvg(imgSrc(i)); c.lcp=true; const extra=(it.imgs||[]).slice(1).map(x=>linked(imgTag(x.img,{cls:'illu illu-lg'}),x)).join('');
      return {cls:'hero',section:'hero',intent:'name the page',layout:'bento-cells',media:svg?'illustration':'image',module:'page-title',inner:`<div class="bento hero-bento"><div class="card ${fill} hero-card"><div class="card-body">${text}</div></div>${mediaCard(i,{cls:'hero-media',eager:true,link:it.img.link,extra})}</div>`}; }
    return {cls:'hero',section:'hero',intent:'name the page',layout:'bento-cell',module:'page-title',inner:`<div class="bento hero-bento"><div class="card ${fill} hero-card hero-card--wide"><div class="card-body">${text}</div></div></div>`}; },
  doors(it,c){ const ds=it.roots.map(doorData); const doors=ds.filter(d=>d.type!=='small'); const smalls=ds.filter(d=>d.type==='small'); const eager=!c.lcp; c.lcp=true;
    const doorHtml=doors.map((d,i)=>`<li class="card card--tint door is-link">${d.img?`<img class="card-image" src="${asset(d.img)}" alt="${esc(d.alt)}" width="768" height="432" loading="${eager&&i<2?'eager':'lazy'}"${eager&&i===0?' fetchpriority="high"':''} decoding="async">`:''}<div class="card-body"><h2 class="card-title h3"><a class="cover-link" href="${esc(d.href)}">${esc(d.title)}</a></h2>${d.text?`<p>${esc(d.text)}</p>`:''}</div></li>`).join('');
    const smallHtml=smalls.map(d=>`<li class="card card--tint door small-door is-link"><div class="card-body">${d.img?`<img class="illu tile-icon" src="${asset(d.img)}" alt="${esc(d.alt)}" width="48" height="48" loading="lazy" decoding="async">`:''}<h2 class="card-title h3"><a class="cover-link" href="${esc(d.href)}">${esc(d.title)}</a></h2>${d.text?`<p>${esc(d.text)}</p>`:''}</div></li>`).join('');
    return {cls:'doors',section:'doors',intent:'route to sub-topics',layout:'grid',items:ds.length,module:'visual-nav',media:'image',inner:`${titleHtml(it)}${doors.length?`\n    <ul class="bento door-grid ${gridCls(doors.length)}" data-slot="cards">${doorHtml}</ul>`:''}${smalls.length?`\n    <ul class="bento small-grid ${gridCls(smalls.length)}" data-slot="cards-small">${smallHtml}</ul>`:''}\n  `}; },
  shortcuts(it){ const links=it.links.map(a=>`<li><a class="arrow shortcut" href="${esc(a.getAttribute('href'))}">${esc(norm(a.textContent))}</a></li>`).join(''); return {cls:'shortcuts',section:'shortcuts',intent:'self-service shortcuts',layout:'bento-cell',items:it.links.length,module:'shortcut-row',inner:`<div class="bento"><div class="card card--syrin shortcuts-card"><div class="card-body">${it.h?`<h2 class="h2-s" data-slot="heading">${esc(norm(it.h.textContent))}</h2>`:''}<ul class="link-list shortcut-list" data-slot="links">${links}</ul></div></div></div>`}; },
  cobranding(it){ const cb=it.el; const cbQ=cb.querySelector('.cobranding__header .ffe-h4, .cobranding__header h2, .cobranding__header h3, .cobranding__header p'); const cbBtn=cb.querySelector('.cobranding__header button'); const cbLogo=cb.querySelector('.cobranding__image-top img'); const cbH=cb.querySelector('.cobranding__heading h2, .cobranding__heading h3, .cobranding__column h2, .cobranding__column h3'); const cbP2=cb.querySelector('.cobranding__column2 .text-wrapper p'); const cbPs=[...cb.querySelectorAll('.cobranding__column .text-wrapper p, .cobranding__text .text-wrapper p')].filter(p=>hasText(p)&&p!==cbP2); const cbAs=[...cb.querySelectorAll('.cobranding__column a[href], .cobranding__content a[href]')].filter(a=>!a.closest('.bank-choice-overlay')); const cbIllu=cb.querySelector('.cobranding__column2 img, .cobranding__image img'); const illuPhoto=cbIllu&&!isSvg(imgSrc(cbIllu));
    ...
    return {cls:'membership',section:'membership',intent:'membership benefits',layout:'bento-cell',module:'cobranding',inner:`<div class="bento"><div class="card card--tint cobrand-card"><div class="card-body">
  ...
  columns(it,c){
    ...
    const help=cols.map(col=>colCard(col,c,it.title?2:1)).join('');
    return {cls:'help',section:'help',intent:'help columns',layout:'grid',items:cols.length,module:it.module==='columns'?'split-media':it.module,media:cols.some(col=>col.img)?'image':undefined,inner:`${it.title?`<h2 class="h2-l section-title" data-slot="heading">${esc(norm(it.title.textContent))}</h2>`:''}<ul class="bento help-grid ${gridCls(cols.length)}" data-slot="cards">${help}</ul>`}; },
  split(it,c){ const im=it.media.img.img; const svg=isSvg(imgSrc(im)); const extra=it.media.parts.slice(1).filter(p=>p.type==='image').map(p=>linked(imgTag(p.it.img,{cls:'illu illu-lg'}),p.it)).join('');
    const single=it.text.ctas.length===1; let body=colParts(it.text,c); if(single) body=coverCta(body);
    return {cls:`split${it.mediaFirst?' media-first':''}`,section:'split',intent:'text with media',layout:'bento-cells',media:svg?'illustration':'image',module:it.module==='columns'?'split-media':it.module,inner:`<div class="bento split-bento${it.mediaFirst?' media-first':''}">${mediaCard(im,{cls:'split-media',link:it.media.img.link,extra})}<div class="card card--tint split-card${single?' is-link':''}"><div class="card-body">${it.title?`<h2 class="h2-m" data-slot="heading">${esc(norm(it.title.textContent))}</h2>`:''}${(it.lead||[]).filter(hasText).map(p=>`<p class="lead">${inline(p)}</p>`).join('')}${body}</div></div></div>`}; },
  cards(it,c){ const photo=it.cards.some(cd=>{const i=cd.querySelector('img'); return i&&imgSrc(i)&&!isSvg(imgSrc(i));});
    const tiles=it.cards.map(cd=>cardTile(cd,c,it.title?3:2)).join('');
    const extras=(it.extraImgs||[]).map(x=>linked(imgTag(x.img,{cls:'illu illu-lg'}),x)).join('')+(it.extraTexts||[]).map(t=>`<div class="prose cards-note">${R(t.w)}</div>`).join('')+((it.extraCtas||[]).length?`<p class="cta-row cards-ctas">${it.extraCtas.map(a=>btn(a)).join('')}</p>`:'');
    return {cls:'popular',section:'popular',intent:'related content',layout:'grid',items:it.cards.length,module:it.module,media:photo?'image':undefined,inner:`${it.title?`<h2 class="h2-l section-title" data-slot="heading">${esc(norm(it.title.textContent))}</h2>`:''}${(it.lead||[]).filter(hasText).map(p=>`<p class="lead section-lead" data-slot="text">${inline(p)}</p>`).join('')}<ul class="bento ${photo?'card-grid':'pop-grid'} ${gridCls(it.cards.length)}" data-slot="cards">${tiles}</ul>${extras}`}; },
  banner(it){ const ps=it.promos||[it];
    if(ps.length>1) return {cls:'switch invites',section:'invites',intent:'invitations',layout:'full-bleed-grid',items:ps.length,module:'promo-band',container:'full-bleed',inner:promoRow(ps)};
    return {cls:'switch',section:'switch',intent:'invitation',layout:'grid',items:1,module:'promo-band',inner:promoRow(ps)}; },
  ...
  compare(it,c){ if(it.el.querySelector('.columns-grid, .image, .card, .button')){ return {cls:'compare',section:'compare',intent:'reference',layout:'bento-cell',module:'cta-band',inner:`<div class="bento"><div class="card card--frost compare-card compare-card--wide"><div class="card-body cta-band-wide">${renderInner(collect(it.el,c),c)}</div></div></div>`}; } const ps=it.ps.map(p=>`<p data-slot="text">${p.innerHTML.replace(/<a\s([^>]*)>/g,'<a $1 class="link-more" rel="noopener">').replace(/<\/a>/g,`${icons.external}</a>`).replace(/\s+/g,' ').trim()}</p>`).join(''); return {cls:'compare',section:'compare',intent:'regulatory: compare prices',layout:'bento-cell',module:'cta-band',inner:`<div class="bento"><div class="card card--frost compare-card"><div class="card-body">${it.h?`<h2 class="h2-s" data-slot="heading">${esc(norm(it.h.textContent))}</h2>`:''}${ps}</div></div></div>`}; },
  faq(it,c){ const qa=it.qs.map(q=>{ const qt=norm(q.querySelector('.ffe-accordion-item__heading-button-content')?.childNodes[0]?.textContent||q.querySelector('h2,h3,h4,button')?.textContent); const body=q.querySelector('.ffe-accordion-item__body')||q.querySelector('.accordion__content, .faq-item__content'); return `<details><summary><h3 class="faq-q">${esc(qt)}</h3>${icons.down}</summary><div class="answer answer-wide">${body?faqBody(body,{demote:2,tidy:true,tidyBase:3,aTrim:'move',emptyB:'space'}):''}</div></details>`; }).join('\n');
    return {cls:'faq-movement',section:'faq',intent:'answers to common questions',module:'faq',items:it.qs.length,container:'container faq-wrap',inner:`${it.title?`<h2 class="h2-l section-title" data-slot="heading">${esc(norm(it.title.textContent))}</h2>`:''}${(it.lead||[]).filter(hasText).map(p=>`<p class="lead section-lead">${inline(p)}</p>`).join('')}<div class="faq" data-slot="items">${qa}</div>`}; },
  ...
  steps(it,c){ const lis=it.steps.map((s,i)=>{ const id=(s.a?.getAttribute('href')||'').replace(/^#/,''); const body=s.body&&hasText(s.body)?s.body:(i===0?it.info:null); const inner=body?renderInner(collect(body,c),c):''; return `<li class="step"${id?` id="${esc(id)}"`:''}><h3 class="step-title title-sm">${s.a?`<a href="${esc(s.a.getAttribute('href'))}">${esc(s.title)}</a>`:esc(s.title)}</h3><div class="step-body">${inner}</div></li>`; }).join('');
    return {cls:'steps',section:'steps',intent:'do the task step by step',layout:'bento-cell',module:'steps',items:it.steps.length,inner:`<div class="bento"><div class="card card--frost steps-card"><div class="card-body">${titleHtml(it,'h2-m steps-h')}<ol class="step-list" data-slot="steps">${lis}</ol></div></div></div>`}; },
  ...
  usp(it){ return {cls:'usp',section:'usp',intent:'benefits',layout:'grid',module:'usp',items:it.entries.length,inner:`${titleHtml(it)}<ul class="bento usp-list ${gridCls(it.entries.length)}" data-slot="cards">${it.entries.map(e=>`<li class="card card--tint usp-item"><div class="card-body">${e.img&&imgSrc(e.img)?imgTag(e.img,{cls:'illu',w:72,h:72}):''}<div class="prose">${R(e.w).replace(/^<p>([^<]{2,60})<\/p>/,`<h${it.title?3:2} class="h3">$1</h${it.title?3:2}>`)}</div></div></li>`).join('')}</ul>`}; },
  ...
  chatColumns(it,c){ const cols=it.cols; const text=cols.flatMap(col=>col.texts).map(t=>R(t.w)).join(''); const img=cols.map(col=>col.img).find(Boolean); const chat=H.chat({el:it.chat},c); if(img) c.lcp=true; return {cls:'ask-movement hero',section:'ask',intent:'ask for help: chat first',layout:img?'bento-cells':'bento-cell',media:img?(isSvg(imgSrc(img.img))?'illustration':'image'):undefined,module:'hero-ask',dyn:chat.dyn,inner:`<div class="bento hero-bento"><div class="card card--frost hero-card hero-ask-card${img?'':' hero-card--wide'}"><div class="card-body"><div class="prose">${text}</div>${chat.inner}${cols.flatMap(col=>col.ctas).map(a=>`<p>${btn(a)}</p>`).join('')}</div></div>${img?mediaCard(img.img,{cls:'hero-media',eager:true,link:img.link}):''}</div>`}; },
...
export function colParts(col,c,base=2){ let out=''; let ctaBuf=[]; const flush=()=>{ if(ctaBuf.length){ out+=ctaBuf.length===1?`<p>${btn(ctaBuf[0],' data-slot="cta"')}</p>`:`<p class="cta-row">${ctaBuf.map(a=>btn(a)).join('')}</p>`; ctaBuf=[]; } };
  for(const p of col.parts){ if(p.type==='cta'){ ctaBuf.push(...p.it.as); continue; } flush();
    ...
    if(p.type==='text'){ const t=p.it; const bl=t.blocks; if(bl.length===2&&/^H[2-4]$/.test(bl[0].tagName)&&!bl[0].querySelector('a,img')&&bl[1].tagName==='P'&&!bl[1].querySelector('a,b,strong,br,img')) out+=`<h2 data-slot="heading">${esc(norm(bl[0].textContent))}</h2><p data-slot="text">${esc(norm(bl[1].textContent))}</p>`; else out+=`<div class="prose">${R(t.w,{tidyBase:base})}</div>`; continue; }
    out+=renderInner([p.it],c); }
  flush(); return out; }
export function cardTile(cd,c,hl=3){ const i=cd.querySelector('img'); const src=imgSrc(i); const a=cd.querySelector('a.card__title, a.ffe-text-link, .card__container-content a[href]'); const tw=cd.querySelector('.card__container-content .text-wrapper'); const pt=!a?cd.querySelector('.card__title'):null; const ps=[...cd.querySelectorAll('p.ffe-card-body__text, .card__container-content-wrapper > p')].filter(x=>x!==pt&&hasText(x)); const tag=cd.querySelector('span.card__tag'); const date=cd.querySelector('span.card__date'); const ctas=[...cd.querySelectorAll('.button a[href], a.ffe-button')].filter(x=>x!==a&&!x.closest('.bank-choice-overlay'));
  const photo=src&&!isSvg(src);
  const img=!src?'':photo?imgTag(i,{cls:'card-image',w:768,h:432}):`<img class="illu tile-icon" src="${asset(src)}" alt="" aria-hidden="true" width="48" height="48" loading="lazy" decoding="async">`;
  let title=''; let body=''; let ctaHtml='';
  if(a) title=`<h${hl} class="card-title h3"><a class="cover-link" href="${esc(a.getAttribute('href'))}">${esc(norm(a.textContent))}</a></h${hl}>`; else if(pt) title=`<h${hl} class="card-title h3">${esc(norm(pt.textContent))}</h${hl}>`;
  if(tw){ const h=tw.querySelector('h1,h2,h3,h4,h5,h6'); const cl=tw.cloneNode(true); if(h&&!a){ const link=ctas[0]; title=`<h${hl} class="card-title h3">${link?`<a class="cover-link" href="${esc(link.getAttribute('href'))}">${esc(norm(h.textContent))}</a>`:esc(norm(h.textContent))}</h${hl}>`; cl.querySelector('h1,h2,h3,h4,h5,h6')?.remove(); if(link) ctaHtml=`<p class="actions">${btn(link,' data-slot="cta"')}</p>`; } if(hasText(cl)) body+=`<div class="prose">${R(cl)}</div>`; }
  body+=ps.map(x=>`<p>${inline(x)}</p>`).join('');
  const rest=ctas.filter(x=>!ctaHtml||x!==ctas[0]); if(rest.length) ctaHtml+=`<p class="actions">${rest.map(x=>btn(x)).join('')}</p>`;
  ...
  const isLink=title.includes('cover-link');
  return `<li class="card card--tint ${photo?'card-tile':'pop-tile'}${isLink?' is-link':''}">${photo?img:''}<div class="card-body">${photo?'':img}${title}${meta}${body}${ctaHtml}</div></li>`; }
...
/* ── round 01 card language (shared by the category-hub, kundeservice-hub, tool and utility families) ── */
/* hero bento: text card 5 + media card 7; text-only hero = one card across 12 */
.hero{padding-top:24px}.hero-bento:has(.hero-media){grid-template-rows:minmax(440px,auto)}
.hero-card{grid-column:1/span 5}.hero-card--wide{grid-column:1/-1}.hero-card .card-body{justify-content:center;padding:56px 48px}
.hero-card .back{margin:0 0 20px}.hero-card .backlink{color:var(--vann)}.hero-card h1,.hero-card .prose h2{font-family:var(--heading-font-family);font-size:var(--t-headline);line-height:1.04;max-width:20ch}.hero-card--wide h1{max-width:26ch}.hero-card--wide .card-body{padding:48px}
/* secondary button on a Frost card: white fill so it keeps its shape (request for chrome) */
.card--frost .btn-secondary{background:#fff}.card--frost .btn-secondary:hover{background:var(--frost-70)}
.hero-card .lead{margin-top:20px}.hero-card .lead+.lead,.hero-card .lead+p{margin-top:12px}.hero-card .cta-row{margin-top:28px}.hero-card .prose{max-width:none}
.hero-bento:has(.hero-card:hover) .media-photo img{transform:scale(1.03)}
.media-photo{grid-column:span 7;margin:0;background:var(--frost-30);min-height:0;position:relative}.media-photo .img-link,.media-photo .media-img{position:absolute;inset:0;display:block;width:100%;height:100%}.media-photo .media-img{object-fit:cover;transition:transform .7s var(--ease)}
.media-illu{grid-column:span 7;margin:0;align-items:center;justify-content:center;padding:32px;gap:16px}.media-illu .illu{width:min(70%,360px);height:auto;max-height:360px;object-fit:contain;transform:none}.media-illu .illu-lg{width:112px;height:112px}
/* doors (visual-nav): photo cards, then icon cards, as one bento under the hero */
.hero+.doors{padding-top:var(--card-gap)}.hero+.doors .section-title{margin-top:var(--section-padding)}
.door .card-body{padding:28px 28px 32px}.door p{color:var(--koksgraa)}.small-door .card-body{padding:28px 28px 32px}.tile-icon{width:48px;height:48px;margin-bottom:6px}
.card .card-title a{color:inherit;text-decoration:none}
/* shortcuts: one Syrin card with the links in columns */
.shortcuts-card{grid-column:1/-1}.shortcuts-card .card-body{padding:36px 48px 40px}
.shortcut-list{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px 24px;margin-top:20px}.shortcut-list li+li{margin-top:0}
/* membership (cobranding) in one Sand card */
.cobrand-card{grid-column:1/-1}.cobrand-card .card-body{padding:36px 48px}
...
.cobrand-panel{display:grid;grid-template-columns:7fr 5fr;gap:var(--spacing-xl);align-items:start;margin-top:var(--spacing-xl)}
...
.cobrand-side{display:grid;gap:var(--spacing-md);justify-items:start;align-self:start;max-width:28rem}.cobrand-illu{width:140px;height:auto}
.cta-row{display:flex;flex-wrap:wrap;align-items:center;gap:var(--spacing-sm) var(--spacing-md)}
/* help columns: one card per captured column (photo bleeds) */
.help-col .card-body{padding:36px 40px 40px}.help-col h2{font-family:var(--title-font-family);font-size:var(--t-title);line-height:1.14}.help-col p{max-width:60ch}.help-col .illu-lg{width:112px;height:112px}.help-col .prose{max-width:none}.help-col .btn-inline{padding-inline:0}
/* split: media card 5 + text card 7 */
.split-bento .media-photo,.split-bento .media-illu{grid-column:span 5}.split-bento .media-photo{min-height:380px}
.split-card{grid-column:span 7}.split-card .card-body{justify-content:center;padding:56px 48px}.split-card h2,.split-card .prose h2{font-family:var(--heading-font-family);font-size:var(--t-headline-sm);line-height:1.05;margin:0}.split-card .prose{max-width:60ch}.split-card .prose h2+p{margin-top:16px}
.split-bento:not(.media-first) .media-photo,.split-bento:not(.media-first) .media-illu{order:2}
.split-bento:has(.split-card:hover) .media-photo img{transform:scale(1.03)}
/* promo (banner): illustration on the tint beside the teaser */
.promo{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center;min-height:240px}.promo--noart{grid-template-columns:minmax(0,1fr)}
.promo .promo-illu{width:180px;max-width:100%;height:auto;margin:24px 0 24px 40px;transition:transform .5s var(--spring)}.promo:hover .promo-illu,.promo:focus-within .promo-illu{transform:scale(1.06) rotate(-2deg)}
.promo .card-body{justify-content:center;padding-inline:40px}.promo p{max-width:60ch}
/* card rails (popular / tips / related): photo cards and icon cards */
.card-tile .card-body{padding:28px 28px 32px}.pop-tile .card-body{padding:24px 24px 28px}.pop-tile .tile-icon{width:48px;height:48px}.card-tile .prose,.pop-tile .prose{max-width:none}.card-tile .prose h2,.card-tile .prose h3,.card-tile .prose h4,.pop-tile .prose h2{margin-top:0;font-size:var(--lead)}.card p,.tile p{color:var(--koksgraa)}
.meta{display:flex;flex-wrap:wrap;gap:4px 12px;color:var(--koksgraa);font-size:var(--label);font-family:var(--title-font-family)}
.section-lead{margin:-8px auto var(--spacing-lg);text-align:center}.cards-note{margin-top:var(--spacing-lg)}.cards-ctas{margin-top:var(--spacing-lg);justify-content:center}
/* compare: Frost card, half width */
.compare-card{grid-column:1/span 6}.compare-card p{margin-top:12px;max-width:52ch}.compare-card--wide{grid-column:1/-1}
.link-more svg{width:16px;height:16px;margin-left:2px;vertical-align:-2px;display:inline-block}
/* faq: centred title, questions in the container at ≤ 52rem */
.faq-wrap .faq{max-width:52rem;margin-inline:auto}.faq-wrap .section-lead{max-width:52rem}
.faq-q{font:inherit;color:inherit;margin:0;flex:1 1 auto;letter-spacing:inherit}
/* steps: one Frost card */
.steps-card{grid-column:1/-1}.steps-card .card-body{padding:48px}.steps-h{margin-bottom:8px}
.step-list{list-style:none;counter-reset:step;padding:0;margin:0;display:grid;gap:0;max-width:68ch}
.step{counter-increment:step;display:grid;grid-template-columns:36px minmax(0,1fr);grid-template-areas:"n title" ". body";gap:var(--spacing-sm) var(--spacing-md);align-items:start;padding:var(--spacing-md) 0;border-top:1px solid rgba(0,39,118,.12)}.step:last-child{border-bottom:1px solid rgba(0,39,118,.12)}
...
/* usp: benefit cards */
.usp-item .card-body{padding:28px}.usp-item .illu{width:64px;height:64px;margin-bottom:6px}.usp-item .prose h2,.usp-item .prose h3{margin:0}.usp-item .prose h2+p,.usp-item .prose h3+p{margin-top:var(--spacing-xs)}
/* chat field (kundeservice ask) inside a Frost card */
.ask{display:grid;gap:6px;margin-top:var(--spacing-lg);max-width:36rem}.ask-row{display:flex;gap:8px;flex-wrap:wrap}
.ask-input{flex:1 1 14rem;min-height:48px;padding:12px 16px;border:0;border-radius:var(--radius-sm);font:var(--body)/1.35 var(--body-font-family);color:var(--color-fg);background:#fff;resize:none}
.ask-input:disabled{color:var(--koksgraa);background:#fff;box-shadow:inset 0 0 0 1px var(--lysgraa);cursor:not-allowed}.ask-input:disabled::placeholder{color:var(--koksgraa)}.ask-note{margin-top:2px}
.ask-input:focus-visible{outline:2px solid var(--vann);outline-offset:2px}
/* callout, feedback */
...
.switch,.compare{padding-top:var(--section-padding)}
@media (max-width:1024px){
  .hero-bento,.hero-bento:has(.hero-media){grid-template-rows:auto}.hero-card{grid-column:1/-1}.hero-bento .media-photo,.hero-bento .media-illu{grid-column:1/-1;order:-1}.hero-bento .media-photo{aspect-ratio:16/9}
  .hero-card .card-body{padding:40px 32px}
  .shortcut-list{grid-template-columns:1fr 1fr}
  .cobrand-panel{grid-template-columns:1fr}
  .split-bento .media-photo,.split-bento .media-illu,.split-card{grid-column:1/-1}.split-bento .media-photo{min-height:0;aspect-ratio:16/9}.split-bento:not(.media-first) .media-photo,.split-bento:not(.media-first) .media-illu{order:-1}
  .split-card .card-body{padding:40px 32px}
  .compare-card{grid-column:1/-1}
  .steps-card .card-body{padding:40px 32px}
}
@media (max-width:767px){
  .hero-card .card-body,.hero-card--wide .card-body,.split-card .card-body,.steps-card .card-body{padding:36px 20px}.hero-card .cta-row{margin-top:24px}
  .shortcut-list{grid-template-columns:1fr}.shortcuts-card .card-body,.cobrand-card .card-body{padding:32px 20px}
  .promo{grid-template-columns:1fr;min-height:0}.promo .promo-illu{margin:24px 20px 0;width:120px}.promo .card-body{padding-inline:20px}
  .help-col .card-body{padding:28px 20px 32px}
  .pop-tile .card-body{flex-direction:row;align-items:center;gap:16px;padding:20px}.pop-tile .tile-icon{margin:0;flex:none}.pop-tile .card-body>*+*{margin-top:0}
  .hero-card .btn,.split-card .btn,.promo .btn,.help-col .btn,.steps-card .btn,.ask-row .btn,.cards-ctas .btn{width:100%;padding:17px 20px}
  .media-illu{padding:24px}.media-illu .illu{width:min(60%,240px);max-height:240px}
...
.help-col .prose{max-width:none}.help-col .illu-lg{width:100%;max-width:200px;height:auto;aspect-ratio:auto}.help-col .btn-inline{padding-inline:0}
.popular .cta-row{margin-top:var(--spacing-lg)}
...
@media (max-width:640px){.cols{grid-template-columns:1fr}.conv-row{grid-template-columns:1fr}.faq summary{font-size:var(--lead)}}
`;
