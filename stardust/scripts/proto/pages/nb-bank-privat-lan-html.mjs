...
export const H={
  ...
  columns(it,c){
    ...
    const help=cols.map(col=>`<article class="help-col">${colParts(col,c,it.title?2:1)}</article>`).join('');
  ...
  usp(it){ return {cls:'usp',section:'usp',intent:'benefits',layout:'grid',module:'usp',items:it.entries.length,inner:`${titleHtml(it)}<ul class="usp-list" data-slot="items">${it.entries.map(e=>`<li class="usp-item">${e.img&&imgSrc(e.img)?imgTag(e.img,{cls:'illu',w:72,h:72}):''}<div class="prose">${R(e.w).replace(/^<p>([^<]{2,60})<\/p>/,`<h${it.title?3:2} class="title-sm">$1</h${it.title?3:2}>`)}</div></li>`).join('')}</ul>`}; },
...
export function cardTile(cd,c,hl=3){ const i=cd.querySelector('img'); const src=imgSrc(i); const a=cd.querySelector('a.card__title, a.ffe-text-link, .card__container-content a[href]'); const tw=cd.querySelector('.card__container-content .text-wrapper'); const pt=!a?cd.querySelector('.card__title'):null; const ps=[...cd.querySelectorAll('p.ffe-card-body__text, .card__container-content-wrapper > p')].filter(x=>x!==pt&&hasText(x)); const tag=cd.querySelector('span.card__tag'); const date=cd.querySelector('span.card__date'); const ctas=[...cd.querySelectorAll('.button a[href], a.ffe-button')].filter(x=>x!==a&&!x.closest('.bank-choice-overlay'));
  ...
  if(a) title=`<h${hl} class="card-title title-sm"><a href="${esc(a.getAttribute('href'))}">${esc(norm(a.textContent))}</a></h${hl}>`; else if(pt) title=`<h${hl} class="card-title title-sm">${esc(norm(pt.textContent))}</h${hl}>`;
  if(tw){ const h=tw.querySelector('h1,h2,h3,h4,h5,h6'); const cl=tw.cloneNode(true); if(h&&!a){ const link=ctas[0]; title=`<h${hl} class="card-title title-sm">${link?`<a href="${esc(link.getAttribute('href'))}">${esc(norm(h.textContent))}</a>`:esc(norm(h.textContent))}</h${hl}>`; cl.querySelector('h1,h2,h3,h4,h5,h6')?.remove(); if(link) ctaHtml=`<p>${btn(link,' data-slot="cta"')}</p>`; } if(hasText(cl)) body+=`<div class="prose">${R(cl)}</div>`; }
...
.door-grid-3{grid-template-columns:repeat(3,minmax(0,1fr))}
...
.usp-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:var(--spacing-lg) var(--spacing-xl)}.usp-item{display:grid;grid-template-columns:72px minmax(0,1fr);gap:var(--spacing-md);align-items:start;padding-top:var(--spacing-md);border-top:1px solid var(--lysgraa)}.usp-item .prose h2,.usp-item .prose h3{margin:0}.usp-item .prose h2+p,.usp-item .prose h3+p{margin-top:var(--spacing-xs)}.card-tile .prose h2,.pop-tile .prose h2{margin-top:0;font-size:var(--lead)}
...
@media (max-width:1023px){.door-grid-3{grid-template-columns:1fr 1fr}}
@media (max-width:640px){.door-grid-3{grid-template-columns:1fr}.intro-grid{grid-template-columns:1fr}.intro-figure{justify-content:flex-start}.intro-figure .illu-xl{max-height:160px}.card-grid,.help-grid-3,.help-grid-4{grid-template-columns:1fr}.cols{grid-template-columns:1fr}.conv-row{grid-template-columns:1fr}.step{grid-template-columns:32px minmax(0,1fr)}.step::before{width:32px;height:32px}.callout-rich{grid-template-columns:24px 1fr;padding:var(--spacing-md)}.callout-rich svg{width:24px;height:24px}.faq summary{font-size:var(--lead)}}
`;
