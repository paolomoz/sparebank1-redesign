...
function calcMov(el,main,doc,head,n){ const cfg=A(el,'script').map(s=>s.textContent).find(t=>/sparebank1Config/.test(t))||''; if(!cfg) return null;
  const cfgStr=k=>(cfg.match(new RegExp(k+':\\s*"([^"]*)"'))||[])[1]||''; const cfgNum=(block,k)=>{ const b=cfg.slice(cfg.indexOf(block)); return (b.match(new RegExp(k+':\\s*Number\\("([\\d.]+)"\\)'))||[])[1]||''; };
  const nok=v=>Number(v).toLocaleString('nb-NO').replace(/ /g,' ')+' kr'; const applyUrl=(cfg.slice(cfg.indexOf('howMuchLoan')).match(/applyForLoan:\s*"([^"]*)"/)||[])[1]||'#'; const contactUrl=(cfg.slice(cfg.indexOf('howMuchLoan')).match(/contactLoan:\s*"([^"]*)"/)||[])[1]||'#';
  const label=(head.match(/<h2[^>]*>([^<]*)<\/h2>/)||[])[1]||esc(txt(main.querySelector('h1')));
