...
function renderPresse({doc,pj}){
  ...
  const advHtml=advisers.map((list,i)=>`<ul class="bento advisers" data-slot="advisers" data-items="${list.length}">${list.map(a=>`<li class="card card--tint adviser is-link"><div class="card-body"><img class="portrait" src="${asset(a.img)}" alt="" aria-hidden="true" width="128" height="128" loading="lazy" decoding="async"><div class="adviser-text"><h3 class="h3 adviser-name">${esc(a.name)}</h3><p class="adviser-role">${esc(a.role)}</p><p class="num adviser-phone">${esc(a.phone)}</p><p class="adviser-mail"><a class="cover-link" href="${esc(a.mail.getAttribute('href'))}">${esc(norm(a.mail.textContent))}</a></p></div></div></li>`).join('')}</ul>`).join('');
  ...
  const mainHtml=`
<section class="movement hero" data-section="hero" data-intent="who this page is for" data-layout="bento-cells" data-media="image" data-module="campaign">
  <div class="container"><div class="bento hero-bento">
    ...
    <div class="card card--frost hero-card"><div class="card-body"><h1 class="h2-l" data-slot="heading">${esc(cH1)}</h1><p class="lead" data-slot="text">${esc(cLead)}</p></div></div>
  </div></div>
</section>
<section class="movement nasjonale" data-section="nasjonale" data-intent="reach the national press contacts" data-layout="grid" data-module="adviser-list" data-items="${advisers.flat().length}" data-media="image">
  <div class="container">
    <div class="prose nat-text"><h2 class="h2-l" data-slot="heading">${esc(natH)}</h2><p class="lead num" data-slot="text">${natLead}</p>${natSmall?`<p class="small muted">${esc(natSmall)}</p>`:''}</div>
    ${advHtml}
  </div>
</section>
<section class="movement lokale" data-section="lokale" data-intent="route to a local bank's press contact" data-layout="bento-cells" data-module="promo-band" data-media="illustration">
  <div class="container"><div class="bento promo-row">
    <div class="card card--syrin promo-art"><img class="promo-illu" src="${asset(lokIllu.getAttribute('src'))}" alt="" aria-hidden="true" width="200" height="200" loading="lazy" decoding="async"></div>
    <div class="card card--tint promo is-link"><div class="card-body"><div class="lok-text prose">${rte(lokText)}</div><p class="lok-cta actions"><a class="${btnClass(lokBtn)} cover-link" href="${esc(lokBtn.getAttribute('href'))}" data-slot="cta">${esc(norm(lokBtn.textContent))}</a></p></div></div>
  </div></div>
</section>
<section class="movement presserom" data-section="presserom" data-intent="go to the press room" data-layout="bento-cells" data-media="image" data-module="campaign">
  <div class="container"><div class="bento pr-bento">
    <div class="card card--tint pr-card"><div class="card-body"><div class="pr-text prose">${prHtml}</div><p class="pr-ctas cta-row">${prBtns.map((a,i)=>`<a class="${i?'btn-inline':btnClass(a)}" href="${esc(a.getAttribute('href'))}"${ext(a)?' rel="noopener"':''}>${esc(norm(a.textContent))}${ext(a)?icons.external:''}</a>`).join('')}</p></div></div>
    <figure class="card pr-photo" data-slot="image"><img src="${asset(prImg.getAttribute('src'))}" alt="${esc(prImg.getAttribute('alt')||'')}" width="1280" height="853" loading="lazy" decoding="async"></figure>
  </div></div>
</section>
<section class="feedback" data-section="feedback" data-intent="rate the page" data-layout="contained" data-module="feedback">
  ...
  const css=`
/* presse archetype — round 01 card language */
.hero{padding-top:24px}
.hero-bento,.pr-bento{grid-template-rows:minmax(440px,auto)}
.hero-photo{grid-column:1/span 7;margin:0;background:var(--frost-30)}.hero-photo img,.pr-photo img{width:100%;height:100%;object-fit:cover;transition:transform .7s var(--ease)}
.hero-card{grid-column:8/-1}.hero-card .card-body,.pr-card .card-body{justify-content:center;padding:56px 48px}.hero-card .lead{margin-top:20px}
/* national press contacts: intro prose + adviser cards (3 × 4, then 2 × 6) */
.nat-text{display:grid;gap:16px;margin-bottom:24px}.nat-text .lead{margin-top:4px}.nat-text .lead strong{font-family:var(--title-font-family);font-weight:400}
.advisers>.adviser{grid-column:span 4}.advisers[data-items="2"]>.adviser{grid-column:span 6}
.adviser .card-body{flex-direction:row;align-items:center;gap:24px;padding:32px 32px}.adviser .card-body>*+*{margin-top:0}
.portrait{flex:none;width:112px;height:112px;border-radius:50%;object-fit:cover;background:var(--frost);transition:transform .5s var(--spring)}
.adviser:hover .portrait,.adviser:focus-within .portrait{transform:scale(1.04)}
.adviser-text{min-width:0;display:grid;gap:4px}.adviser-role{color:var(--koksgraa)}.adviser-phone{font-family:var(--title-font-family);margin-top:8px}.adviser-mail{margin-top:8px}
/* local press: illustration tile 3 + teaser 9 */
.promo-row .card{min-height:260px}
...
.promo-art img{width:auto;height:auto;max-height:180px;max-width:100%;transition:transform .5s var(--spring)}
.promo-art:has(+.promo:hover) img,.promo-art:has(+.promo:focus-within) img{transform:scale(1.06) rotate(-2deg)}.promo-art:has(+.promo:hover),.promo-art:has(+.promo:focus-within){background:var(--syrin-70)}
.promo{grid-column:span 9}.promo .card-body{justify-content:center;padding-inline:48px}.promo .prose h2{font-size:var(--t-headline-sm);line-height:1.05}.promo .prose p{max-width:60ch;margin-top:12px}.promo .actions{margin-top:24px}
/* press room: text 5 + photo 7 */
.pr-card{grid-column:1/span 5}.pr-text h2{font-size:var(--t-headline-sm);line-height:1.05}.pr-text p{margin-top:16px}.pr-ctas{margin-top:28px}.pr-ctas .btn svg{width:18px;height:18px}
.pr-photo{grid-column:6/-1;margin:0;background:var(--frost-30)}
.pr-bento:has(.pr-card:hover) .pr-photo img{transform:scale(1.03)}
.feedback .feedback-row{justify-content:flex-start}.feedback-btns{display:flex;gap:var(--spacing-sm)}
@media (max-width:1024px){
  .hero-bento,.pr-bento{grid-template-rows:auto}
  .hero-photo,.pr-photo{grid-column:1/-1;aspect-ratio:16/9}.hero-card,.pr-card{grid-column:1/-1}.pr-photo{order:-1}
  .hero-card .card-body,.pr-card .card-body{padding:40px 32px}
  .advisers>.adviser,.advisers[data-items="2"]>.adviser{grid-column:span 6}.adviser .card-body{padding:28px 28px}
  .promo-art{grid-column:span 4}.promo{grid-column:span 8}.promo .card-body{padding-inline:32px}
}
@media (max-width:767px){
  .hero-card .card-body,.pr-card .card-body{padding:36px 20px}
  .advisers>.adviser,.advisers[data-items="2"]>.adviser{grid-column:1/-1}.adviser .card-body{padding:24px 20px;gap:16px}.portrait{width:88px;height:88px}
  .promo-row .card{min-height:0}.promo-art{grid-column:1/-1;padding:20px}.promo-art img{max-height:140px;max-width:60%}.promo{grid-column:1/-1}.promo .card-body{padding-inline:20px}
...
`;
  return { template:'static', title:pj.title, description:norm(pj.metaDescription), main:mainHtml, css,
    provenance:{ shapeBrief:'stardust/prototypes/nb-bank-om-oss-presse-html-shape.md', dominantDimension:'composition/paper-movements', conceptSeed:'surface cb42046c read (dealt 6,2,1; 6 built)', unsourcedContent:[], signatureElements:['photos bleeding in bento cells (hero 7, press room 7)','circle portraits on adviser cards','spot illustration voksen-dame-med-mobil.svg on a Syrin-30 tile'], improvementsApplied:['round 01: hero + press room as bento cells','round 01: adviser cards 3×4 + 2×6, 2 px corners, 6 px gutters','round 01: no movement tints, no shadows','Ramp type scale, 6 px borderless buttons'], dynamicsInterim:['feedback thumbs static (type=button)','"Velg bank" overlay bank picker → plain links to the captured href'], canonDeviations:[] } };
}
...
function cardHtml(card,ctx){
  ...
  const title=a?`<h3 class="card-title h3"><a class="cover-link" href="${esc(a.getAttribute('href'))}"${ext(a)?' rel="noopener"':''}>${esc(norm(a.textContent))}</a></h3>`:'';
  const photo=!!src&&isPhoto(src); const media=src?(photo?`<img class="card-image" src="${asset(src)}" alt="${esc(img.getAttribute('alt')||'')}" width="768" height="432" loading="lazy" decoding="async">`:`<img class="illu" src="${asset(src)}" alt="${esc(img.getAttribute('alt')||'')}"${img.getAttribute('alt')?'':' aria-hidden="true"'} width="72" height="72" loading="lazy" decoding="async">`):'';
  return `<li class="card card--tint${a?' is-link':''}">${photo?media:''}<div class="card-body">${photo?'':media}${title}${text?`<div class="prose">${rich(text.querySelector('.text-wrapper')||text)}</div>`:''}${ps.map(p=>`<p>${esc(norm(p.textContent))}</p>`).join('')}${ctas?ctaHtml(ctas,'ctas actions'):''}</div></li>`;
}
...
function comp(el,ctx){
  ...
  if(cl.contains('static-cards')||cl.contains('card-list')){ const cards=[...el.querySelectorAll('.card')]; return cards.length?`<ul class="bento cards" data-module="cards" data-slot="cards" data-items="${cards.length}">${cards.map(c=>cardHtml(c,ctx)).join('')}</ul>`:''; }
  if(cl.contains('card')&&!cl.contains('card-list')) return `<ul class="bento cards cards-1" data-module="cards" data-slot="cards" data-items="1">${cardHtml(el,ctx)}</ul>`;
  ...
  if(cl.contains('background-container')){ const inner=el.querySelector(':scope > .background-container__wrap > .background-container__content')||el; const html=walk(inner,ctx); if(!html) return ''; const p=paperOf(el); const fill={'paper-sand':'card--tint','paper-frost':'card--frost','paper-syrin':'card--syrin'}[p]||''; const nests=/class="(bento|card)[ "]/.test(html); return fill&&!nests?`<div class="bento"><div class="card ${fill} panel-card" data-module="background-container"><div class="card-body">${html}</div></div></div>`:`<div class="panel" data-module="background-container">${html}</div>`; }
  if(cl.contains('adviser-list')){ const list=[...el.querySelectorAll('.adviser')].map(a=>({img:imgSrc(a.querySelector('img')),name:norm(a.querySelector('.adviser-name')?.textContent),role:norm(a.querySelector('.adviser-subtext.ffe-paragraph:not(.adviser-phone)')?.textContent),phone:norm(a.querySelector('.adviser-phone')?.textContent),mail:a.querySelector('a.adviser-email')})); return `<ul class="bento advisers" data-module="adviser-list" data-slot="advisers" data-items="${list.length}">${list.map(a=>`<li class="card card--tint adviser${a.mail?' is-link':''}"><div class="card-body">${a.img?`<img class="portrait" src="${asset(a.img)}" alt="" aria-hidden="true" width="128" height="128" loading="lazy" decoding="async">`:''}<div class="adviser-text"><h3 class="h3 adviser-name">${esc(a.name)}</h3>${a.role?`<p class="adviser-role">${esc(a.role)}</p>`:''}${a.phone?`<p class="num adviser-phone">${esc(a.phone)}</p>`:''}${a.mail?`<p class="adviser-mail"><a class="cover-link" href="${esc(a.mail.getAttribute('href'))}">${esc(norm(a.mail.textContent))}</a></p>`:''}</div></div></li>`).join('')}</ul>`; }
  ...
  if(cl.contains('table')&&el.querySelector('table')){ const t=el.querySelector('.table-wrapper table')||el.querySelector('table'); return isLayoutTable(t)?walk(t,ctx):`<div class="table-wrap card card--tint" data-module="table"><table>${rich(t)}</table></div>`; }
  if(tag==='TABLE') return isLayoutTable(el)?walk(el,ctx):`<div class="table-wrap card card--tint" data-module="table"><table>${rich(el)}</table></div>`;
  ...
  if(cl.contains('campaign')){ const img=el.querySelector('.campaign-bg img, .campaign__wrap-bg img'); const src=imgSrc(img); const content=el.querySelector('.campaign-content')||el; const eager=src&&!ctx.eagerUsed; if(src) ctx.eagerUsed=true; return `<div class="bento hero-bento${src?'':' hero-noimg'}">${src?`<figure class="card hero-photo" data-slot="image"><img src="${asset(src)}" alt="${esc(img.getAttribute('alt')||'')}" width="1280" height="853" loading="${eager?'eager':'lazy'}"${eager?' fetchpriority="high"':''} decoding="async"></figure>`:''}<div class="card card--frost hero-card"><div class="card-body">${walk(content,ctx)}</div></div></div>`; }
...
function renderGeneric({doc,pj,slug}){
  ...
  for(const g of groups){ if(g.kind==='hr'){ lastTint=lastTint; continue; }
    ...
    let tint=''; if(g.kind==='background-container'){ inner=inner.replace(/^<div class="panel" data-module="background-container">([\s\S]*)<\/div>$/,'$1'); } // round 01: the fill belongs to the card (panel-card), never to the movement
    ...
    const layout=g.kind==='campaign'?'bento-cells':(g.kind==='columns-grid'?'split':(/^<div class="bento"><div class="card/.test(inner)?'bento-cell':(g.kind==='static-cards'||g.kind==='adviser-list'?'grid':'contained')));
  ...
  const css=`
/* om-oss walker — round 01 card language */
.hero{padding-top:24px}
.hero-bento{grid-template-rows:minmax(420px,auto)}.hero-bento.hero-noimg{grid-template-rows:auto}
.hero-photo{grid-column:1/span 7;margin:0;background:var(--frost-30)}.hero-photo img{width:100%;height:100%;object-fit:cover}
.hero-card{grid-column:8/-1}.hero-noimg .hero-card{grid-column:1/-1}.hero-card .card-body{justify-content:center;padding:56px 48px}
.hero-card .prose{max-width:none}.hero-card .prose h1{font-size:var(--t-headline);line-height:1.04;max-width:20ch}.hero-card .prose h1+p{margin-top:20px;font-size:var(--lead);line-height:1.33;max-width:60ch}
...
.col{display:grid;gap:var(--spacing-md);min-width:0}.col>.prose+.prose{margin-top:0}
/* grouped step (background-container): one card, the fill belongs to the card */
.panel{display:grid;gap:var(--spacing-lg)}.movement>.container>.panel{gap:var(--spacing-xl)}
.panel-card{grid-column:1/-1}.panel-card .card-body{display:grid;gap:var(--spacing-lg);padding:40px 48px}.panel-card .card-body>*+*{margin-top:0}
/* cards: bento of tinted cards, spans by count */
.cards>.card{grid-column:span 4}.cards[data-items="1"]>.card,.cards[data-items="2"]>.card{grid-column:span 6}.cards[data-items="4"]>.card,.cards[data-items="8"]>.card{grid-column:span 3}
.col .cards>.card,.col .cards[data-items="1"]>.card,.col .cards[data-items="2"]>.card{grid-column:1/-1}
.cards .card-body{padding:32px 28px 36px}.cards .card .illu{width:56px;height:56px;align-self:flex-start;margin-bottom:6px}.cards .card .prose p+p{margin-top:var(--spacing-sm)}.cards .card .actions{margin-top:20px}.cards .card h3{font-size:var(--title)}
...
/* data table on one tinted card */
.table-wrap{overflow-x:auto;max-width:100%}.table-wrap.card{display:block;padding:20px 24px}.table-wrap table{width:100%;min-width:32rem;border-collapse:collapse;font-size:var(--body-sm)}.table-wrap th,.table-wrap td{padding:12px 16px;border-bottom:1px solid rgba(0,39,118,.12);text-align:left;vertical-align:top}.table-wrap tr:last-child td{border-bottom:0}.table-wrap th{font-family:var(--title-font-family);font-weight:400;color:var(--fjell)}.table-wrap thead td,.table-wrap thead th{font-family:var(--title-font-family);color:var(--fjell)}.table-wrap caption{text-align:left;font-family:var(--title-font-family);padding-bottom:var(--spacing-sm)}
...
.feedback .feedback-row{justify-content:flex-start}.feedback-btns{display:flex;gap:var(--spacing-sm)}
/* people: adviser cards with a round portrait */
.advisers>.adviser{grid-column:span 4}.advisers[data-items="2"]>.adviser{grid-column:span 6}
.adviser .card-body{flex-direction:row;align-items:center;gap:24px;padding:32px 32px}.adviser .card-body>*+*{margin-top:0}
.portrait{flex:none;width:112px;height:112px;border-radius:50%;object-fit:cover;background:var(--frost);transition:transform .5s var(--spring)}.adviser:hover .portrait,.adviser:focus-within .portrait{transform:scale(1.04)}
.adviser-text{min-width:0;display:grid;gap:4px}.adviser-role{color:var(--koksgraa)}.adviser-phone{font-family:var(--title-font-family);margin-top:8px}.adviser-mail{margin-top:8px}
.rich-fallback table{width:100%;border-collapse:collapse}.rich-fallback td,.rich-fallback th{padding:8px 12px;border-bottom:1px solid rgba(0,39,118,.12);vertical-align:top;text-align:left}
@media (max-width:1024px){
  .hero-bento{grid-template-rows:auto}.hero-photo{grid-column:1/-1;aspect-ratio:16/9}.hero-card{grid-column:1/-1}.hero-card .card-body{padding:40px 32px}
  .cols,.cols[data-items="3"],.cols[data-items="4"]{grid-template-columns:1fr;gap:var(--spacing-xl)}.col .fig{max-width:600px}
  .cards>.card,.cards[data-items="4"]>.card,.cards[data-items="8"]>.card{grid-column:span 6}
  .advisers>.adviser,.advisers[data-items="2"]>.adviser{grid-column:span 6}.adviser .card-body{padding:28px 28px}
  .panel-card .card-body{padding:32px 28px}
}
@media (max-width:767px){
  .hero-card .card-body{padding:36px 20px}
  .cards>.card,.cards[data-items="1"]>.card,.cards[data-items="2"]>.card,.cards[data-items="4"]>.card,.cards[data-items="8"]>.card{grid-column:1/-1}.cards .card-body{padding:28px 20px 32px}
  .advisers>.adviser,.advisers[data-items="2"]>.adviser{grid-column:1/-1}.adviser .card-body{padding:24px 20px;gap:16px}.portrait{width:88px;height:88px}
  .panel-card .card-body{padding:32px 20px}
  .ctas .btn{flex:1 1 100%;white-space:normal;text-align:center}
}
@media (max-width:640px){
  .table-wrap.card{padding:12px 12px}.table-wrap table{min-width:0}.table-wrap th,.table-wrap td{padding:8px 10px}
}
`;
  return { template:'static', title:pj.title, description:norm(pj.metaDescription), main:mainHtml, css,
    provenance:{ shapeBrief:'stardust/prototypes/nb-bank-om-oss-presse-html-shape.md', dominantDimension:'composition/paper-movements', conceptSeed:'surface cb42046c read (family walker)', unsourcedContent:[], richTextFallbacks:ctx.fallbacks, ...(ctx.fallbackTags.length?{richTextFallbackComponents:[...new Set(ctx.fallbackTags)]}:{}), signatureElements:['photos bleeding in bento cells','circle portraits on adviser cards','spot illustrations on the card fill'], improvementsApplied:['round 01: hero as bento cells (photo 7 + Frost text 5)','round 01: cards, advisers, tinted panels and tables as 2 px cards with 6 px gutters','round 01: no movement tints — the fill belongs to the card','no <hr>'], dynamicsInterim:['feedback thumbs static (type=button)','"Velg bank" overlay CTAs → plain links to the captured href','tabs → in-page sections with a jump nav (dynamics #11)','accordions → <details>'], canonDeviations:[] } };
}
